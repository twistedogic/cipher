...
fn test_epub_processing_and_lancedb_creation() -> Result<()> {
    ...
    if !executable_path.exists() {
        println!("Executable not found at {:?}, attempting to build...", executable_path);
    ...
    println!("Using Ollama model: {} (dim: {})", ollama_model_name, ollama_embedding_dim);
    ...
    assert!(table_data_path.exists(), "LanceDB table directory not found at {:?}", table_data_path);
    assert!(table_data_path.is_dir(), "LanceDB table path is not a directory: {:?}", table_data_path);
...
fn test_rag_query_command() -> Result<()> {
    ...
    ingest_cmd.arg("ingest")
        .arg("--epub-path").arg(&test_epub_path)
        .arg("--db-path").arg(&temp_db_path)
        .arg("--table-name").arg(ingest_table_name)
        .arg("--ollama-embedding-model").arg(ollama_embedding_model)
        .arg("--embedding-dim").arg(ollama_embedding_dim);
        // .arg("--ollama-url").arg("http://localhost:11434"); // Default is usually fine

    let ingest_output = ingest_cmd.output().expect("Ingest command failed to execute");
    if !ingest_output.status.success() {
        println!("Ingest Stdout:\n{}", String::from_utf8_lossy(&ingest_output.stdout));
        eprintln!("Ingest Stderr:\n{}", String::from_utf8_lossy(&ingest_output.stderr));
    }
    assert!(ingest_output.status.success(), "Ingestion step failed for RAG test.");
    ...
    assert!(!query_output.stdout.is_empty(), "RAG query command produced no output to stdout.");
