...
use ollama_rs::generation::embeddings::EmbeddingsRequest; // For generating embeddings
        ...
        match doc.get_resource(spine_item_id) {
            Ok(content_bytes_vec) => {
                ...
                for text_chunk in text_chunks.iter() {
                    ...
                    // For now, one by one.
                    let request = EmbeddingsRequest::new(args.ollama_model.clone(), text_chunk.clone());
