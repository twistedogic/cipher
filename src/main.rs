...
use arrow_array::{RecordBatch, StringArray, Float32Array, FixedSizeListArray, Int32Array}; // Corrected Int32Array import
...
async fn process_epub_and_embed(args: IngestArgs, ollama: Arc<Ollama>) -> Result<()> {
    ...
    let schema = Arc::new(Schema::new(vec![
        ...
        Field::new(
            "embedding",
            DataType::FixedSizeList(Arc::new(Field::new("item", DataType::Float32, true)), args.embedding_dim),
            true,
        ),
    ...
    let epub_filename = epub_path.file_name().unwrap_or_default().to_string_lossy().into_owned();
    ...
    for (chapter_idx_usize, spine_item_id) in spine_ids.iter().enumerate() {
        ...
        match doc.get_resource(spine_item_id) {
            Ok(content_bytes_vec) => {
                ...
                if html_content.trim().is_empty() { continue; }
                let markdown_text = html2md::parse_html(&html_content);
                if markdown_text.trim().is_empty() { continue; }

                println!("--- Chapter {} (Spine Item: {}) ---", chapter_idx + 1, spine_item_id);
                ...
                println!("Generating embeddings for {} chunks via Ollama...", text_chunks.len());
                ...
                for text_chunk in text_chunks.iter() {
                    if text_chunk.trim().is_empty() { continue; }
                    let request = EmbeddingsRequest::new(args.ollama_embedding_model.clone(), text_chunk.clone());
                    match ollama.generate_embeddings(request).await {
                        Ok(response) => {
                            let embedding_f32: Vec<f32> = response.embeddings.iter().map(|&x| x as f32).collect();
                        ...
                        Err(e) => {
                            eprintln!("Error generating Ollama embedding for chunk '{}': {:?}", text_chunk, e);
                        }
                    }
                }

                if !current_texts_for_db.is_empty() {
                    println!("Embeddings generated for chapter {}. Preparing to save to DB.", chapter_idx + 1);
                    ...
                    let mut embedding_col_builder = arrow_array::builder::FixedSizeListBuilder::new(
                        arrow_array::builder::Float32Builder::new(), args.embedding_dim
                    );
                    ...
                    let mut chunk_idx_in_chapter_col_builder = arrow_array::builder::Int32Builder::new();
                    ...
                    for (idx, text_chunk) in current_texts_for_db.iter().enumerate() {
                        if let Some(embedding_f32) = current_embeddings_for_db.get(idx) {
                            ...
                            if embedding_f32.len() == args.embedding_dim as usize {
                                for val in embedding_f32 { value_builder.append_value(*val); }
                    ...
                    if text_col_builder.len() > 0 {
                        ...
                        println!("Added {} chunks from chapter {} to LanceDB.", current_texts_for_db.len(), chapter_idx + 1);
                    }
                }
...
fn chunk_markdown(markdown: &str) -> Vec<String> {
    ...
    for event in Parser::new(markdown) {
        match event {
            ...
            Event::Start(Tag::Heading(_, _, _)) | Event::Start(Tag::List(_)) | Event::Start(Tag::BlockQuote) => {
                 if !current_chunk.trim().is_empty() { chunks.push(current_chunk.trim().to_string()); }
                 current_chunk.clear();
            }
            Event::End(Tag::Heading(_, _, _)) | Event::End(Tag::List(_)) | Event::End(Tag::BlockQuote) => {
                if !current_chunk.trim().is_empty() { chunks.push(current_chunk.trim().to_string()); }
                current_chunk.clear();
            }
            _ => {}
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Ingest(args) => {
            println!("Initializing Ollama client for Ingest with URL: {}", args.ollama_url);
            let ollama = Arc::new(Ollama::from_url(args.ollama_url.clone()));
            process_epub_and_embed(args, ollama).await?;
        }
        Commands::Query(args) => {
            println!("Initializing Ollama client for Query with URL: {}", args.ollama_url);
            let ollama = Arc::new(Ollama::from_url(args.ollama_url.clone()));
...
async fn handle_query_command(args: QueryArgs, ollama: Arc<Ollama>) -> Result<()> {
    ...
    println!("Using Ollama embedding model: {}", args.ollama_embedding_model);
    println!("Expected embedding dimension: {}", args.embedding_dim);

    // 1. Generate embedding for the query
    let query_embedding_request = EmbeddingsRequest::new(args.ollama_embedding_model.clone(), args.query.clone());
    ...
    let tbl = conn.open_table(&args.table_name)
        .await
        .with_context(|| format!("Failed to open table '{}' in LanceDB at {:?}", args.table_name, db_uri))?;

    // 3. Perform similarity search
    println!("Performing similarity search in LanceDB table '{}'...", args.table_name);
    let search_results = tbl.search(&query_embedding_f32) // Pass Vec<f32>
        .limit(args.top_k)
    ...
    if record_batches.is_empty() {
        ...
        for batch in record_batches {
            let text_array = batch
                ...
                .ok_or_else(|| anyhow::anyhow!("Failed to downcast 'text' column to StringArray"))?;
            ...
            for i in 0..text_array.len() {
                if retrieved_chunks_text.len() < args.top_k { // Ensure we don't exceed top_k due to batching
                    let text_val = text_array.value(i).to_string();
                    println!("---\nChunk {}:\n{}\n---", retrieved_chunks_text.len() + 1, text_val);
    ...
    println!("\n--- Sending prompt to Ollama generation model: {} ---", args.ollama_generation_model);
    ...
    match ollama.generate_completion(gen_request).await {
        ...
        Err(e) => {
            eprintln!("Error generating response from Ollama model '{}': {:?}", args.ollama_generation_model, e);
